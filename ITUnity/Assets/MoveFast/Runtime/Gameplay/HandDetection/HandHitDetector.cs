/*
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 * All rights reserved.
 *
 * Licensed under the Oculus SDK License Agreement (the "License");
 * you may not use the Oculus SDK except in compliance with the License,
 * which is provided at the time of installation or download, or which
 * otherwise accompanies this software in either electronic or hard copy form.
 *
 * You may obtain a copy of the License at
 *
 * https://developer.oculus.com/licenses/oculussdk/
 *
 * Unless required by applicable law or agreed to in writing, the Oculus SDK
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

using Oculus.Interaction.Input;
using System;
using System.Collections;
using UnityEngine;
using UnityEngine.Events;

namespace Oculus.Interaction.MoveFast
{
    /// <summary>
    /// Determines if the Hand is in the right pose as it collides with the object
    /// </summary>
    public class HandHitDetector : MonoBehaviour
    {
        [SerializeField]
        private string _fistPoseName;

        [SerializeField]
        private string _palmPoseName;

        [SerializeField]
        private string _chopPoseName;

        [SerializeField]
        float _minTimeBetweenHits = 0f;

        [SerializeField]
        int _poseFrameTolerance = 3;

        public UnityEvent onHit;

        public UnityEvent onFistLeft;
        public UnityEvent onFistRight;
        public UnityEvent onPalmLeft;
        public UnityEvent onPalmRight;
        public UnityEvent onChopLeft;
        public UnityEvent onChopRight;

        private TriggerZoneList<IHand> _triggerZone;
        private float _lastHitTime;

        /// <summary>
        /// The hand that most recently hit this target
        /// </summary>
        public IHand LastHand { get; private set; }

        private void Start()
        {
            _triggerZone = new TriggerZoneList<IHand>(gameObject.AddComponent<TriggerZone>());
            _triggerZone.WhenAdded += TestForHitPose;
        }

        private void OnDestroy()
        {
            _triggerZone.Dispose();
            _triggerZone = null;
        }

        private void TestForHitPose(IHand hand)
        {
            //if (!_canHit) return;
            if (Time.time - _lastHitTime < _minTimeBetweenHits) return;

            _lastHitTime = Time.time;
            LastHand = hand;

            IngameHitFist(hand);
            IngameHitPalm(hand);
            IngameHitChop(hand);
        }

        /// <summary>
        /// Hits regardless to the pose, resolves true if the hand was in the right pose
        /// </summary>
        /// <param name="hand"></param>
        private void IngameHitFist(IHand hand)
        {
            bool hasPoseList = hand.TryGetAspect<HandPoseActiveStateList>(out var handPoseList);

            // hand poses are not available, probably a controller, just say it was posed right
            if (!hasPoseList)
            {
                return;
            }

            StartCoroutine(Tolerance());
            IEnumerator Tolerance()
            {
                var handPoseActiveState = handPoseList.Get(_fistPoseName);

                if (handPoseActiveState != null)
                {
                    for (int i = 0; i < _poseFrameTolerance; i++)
                    {
                        // hand was in the right pose \o/
                        if (handPoseActiveState.Active)
                        {
                            onHit?.Invoke();

                            if (handPoseList.isLeft)
                            {
                                onFistLeft?.Invoke(); 
                            }
                            else
                            {
                                onFistRight?.Invoke(); 
                            }
                            yield break;
                        }
                        yield return null;
                    }
                }
            }
        }

        /// <summary>
        /// Hits regardless to the pose, resolves true if the hand was in the right pose
        /// </summary>
        /// <param name="hand"></param>
        private void IngameHitPalm(IHand hand)
        {
            bool hasPoseList = hand.TryGetAspect<HandPoseActiveStateList>(out var handPoseList);

            // hand poses are not available, probably a controller, just say it was posed right
            if (!hasPoseList)
            {
                return;
            }

            StartCoroutine(Tolerance());
            IEnumerator Tolerance()
            {
                var handPoseActiveState = handPoseList.Get(_palmPoseName);

                if (handPoseActiveState != null)
                {
                    for (int i = 0; i < _poseFrameTolerance; i++)
                    {
                        // hand was in the right pose \o/
                        if (handPoseActiveState.Active)
                        {
                            onHit?.Invoke();

                            if (handPoseList.isLeft)
                            {
                                onPalmLeft?.Invoke();
                            }
                            else
                            {
                                onPalmRight?.Invoke();
                            }
                            yield break;
                        }
                        yield return null;
                    }
                }
            }
        }

        /// <summary>
        /// Hits regardless to the pose, resolves true if the hand was in the right pose
        /// </summary>
        /// <param name="hand"></param>
        private void IngameHitChop(IHand hand)
        {
            bool hasPoseList = hand.TryGetAspect<HandPoseActiveStateList>(out var handPoseList);

            // hand poses are not available, probably a controller, just say it was posed right
            if (!hasPoseList)
            {
                return;
            }

            StartCoroutine(Tolerance());
            IEnumerator Tolerance()
            {
                var handPoseActiveState = handPoseList.Get(_chopPoseName);

                if (handPoseActiveState != null)
                {
                    for (int i = 0; i < _poseFrameTolerance; i++)
                    {
                        // hand was in the right pose \o/
                        if (handPoseActiveState.Active)
                        {
                            onHit?.Invoke();

                            if (handPoseList.isLeft)
                            {
                                onChopLeft?.Invoke();
                            }
                            else
                            {
                                onChopRight?.Invoke();
                            }
                            yield break;
                        }
                        yield return null;
                    }
                }
            }
        }
    }
}
