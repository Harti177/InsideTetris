public class IBlockHorizontal : TetrisPiece
{
    ...
    public override (bool, int[,]) CalculateFromRotatePosition(int xPosition, int yPosition, int xLength, int yLength, int from)
    {
        int[,] values = new int[4, 2];

        if (from == 2)
        {
            if (yPosition + 1 > yLength - 1)
                return (true, null);

            values[0, 0] = xPosition - 2 < 0 ? xLength - 1 : xPosition - 2;
            values[0, 1] = yPosition + 1;
        }

        if (from == 4)
        {
            if (yPosition + 2 > yLength - 1)
                return (true, null);

            values[0, 0] = xPosition - 1 < 0 ? xLength - 1 : xPosition - 1;
            values[0, 1] = yPosition + 2;
        }
    ...
    {
        values[1, 0] = values[0, 0] + 1 > xLength - 1 ? 0 : values[0, 0] + 1;
        values[1, 1] = values[0, 1];

        values[2, 0] = values[1, 0] + 1 > xLength - 1 ? 0 : values[1, 0] + 1;
        values[2, 1] = values[0, 1];

        values[3, 0] = values[2, 0] + 1 > xLength - 1 ? 0 : values[2, 0] + 1;
