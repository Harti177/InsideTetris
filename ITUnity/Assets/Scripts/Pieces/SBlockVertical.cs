public class SBlockVertical : TetrisPiece
{
    ...
    public override (bool, int[,]) CalculateFromRotatePosition(int xPosition, int yPosition, int xLength, int yLength, int from)
    {
        ...
        if (from == 1)
        {
            ...
            values[0, 0] = xPosition  + 2 > xLength - 1 ? 0 : xPosition + 2;
            values[0, 1] = yPosition - 1;
        }
    ...
    protected override void CalculateRest(ref int[,] values, int xLength, int yLength)
    {
        ...
        values[2, 1] = values[1, 1];

        values[3, 0] = values[2, 0];
        values[3, 1] = values[0, 1] + 2 > yLength - 1 ? -1 : values[0, 1] + 2;
    }
}
